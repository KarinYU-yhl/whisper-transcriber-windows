"""
MLX Whisper Transcriber - Windows port.

This is a Windows adaptation of the macOS MLX Whisper GUI. The user-facing
behaviour (file selection, drag & drop, batch processing, model selection,
language selection, Hugging Face cache management, offline local models,
progress logs, .txt output) is preserved. The Apple-only MLX inference engine
is replaced by faster-whisper (CTranslate2), which runs on Windows CPU and
NVIDIA CUDA GPUs. Audio decoding uses PyAV (bundled FFmpeg), so a separate
...
# download path is more robust. Symlinks are also unavailable on many Windows
# setups, so silence that warning too.
...
class App(ctk.CTk, tkinterdnd2.TkinterDnD.DnDWrapper):
    ...
    def __init__(self):
        ...
        self.title("Whisper Transcriber (Windows)")
        ...
        self.record_status_label = ctk.CTkLabel(
            self.file_frame,
            text="Record Teams / any app audio + your mic, then auto-transcribe.",
            text_color="gray",
            font=ctk.CTkFont(size=12),
        )
    ...
    def show_help(self):
        help_text = (
            ...
            "NOTE: This Windows build uses faster-whisper (CTranslate2) models, NOT the MLX models "
            "used by the macOS version. Use the Systran/faster-whisper-* repositories."
        )
    ...
    def open_source(self):
        if hasattr(self, "current_source") and self.current_source:
            if os.path.isdir(self.current_source):
                os.startfile(self.current_source)  # Windows: open folder in Explorer
            else:
