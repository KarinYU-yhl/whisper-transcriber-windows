"""
Command-line transcription tool for Windows.

Windows port of the macOS mlx-whisper tool. Instead of Apple's MLX engine
(Apple Silicon only), this uses faster-whisper (CTranslate2), which runs on
Windows CPU and NVIDIA CUDA GPUs. Audio decoding is handled by PyAV (bundled
FFmpeg), so a system-wide FFmpeg install is not required.
